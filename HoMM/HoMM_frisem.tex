...
\begin{frame}[standout]
On this task, HoMM is not much worse than humans with much less experience, and is much better than the language baseline.
\end{frame}
...
\begin{frame}[standout]
Why categorization? Cognitive history, richer input, and classification is common.
\end{frame}
...
\begin{frame}[standout]
Meta-mappping works well on classification tasks from pixel input, and can even generalize to unseen mappings.
\end{frame}
...
\begin{frame}[standout]
Why RL? Recent AI achievements, relates to neuroscience and cognition, and requires more sophisticated adaptation. 
\end{frame}
...
\begin{frame}[standout]
Meta-mappping works well in RL, even with a small support set of tasks. 
\end{frame}
...
\begin{frame}[standout]
Starting from a good ``guess'' at the appropriate behavior results in faster learning, and reduces cumulative mistakes by orders of magnitude.
\end{frame}

\section{Wrapping up}

\begin{frame}{Connecting to my recent RL generalization work}
How does this relate to my recent work with collaborators showing realistic environments lead to good generalization in language-conditioned RL?\par
\begin{itemize}[<+(2)->]
\item In most experiments there, the agent had seen most the tasks in training, with only a few held-out. If I do similarly in some of these domains, the language baseline does much better. But do humans really see 95\% of tasks before we're asked to generalize to a new one? 
\item However, that work does motivate trying this in more complex environments in the future, results might indeed differ there. Whether this perspective is computationally useful with data as rich as human experience is an open question.
\end{itemize}
...
\item We propose a parsimonious \textbf{HoMM} architecture that: 
    ...
    \item Uses an analogy between basic tasks and meta-mappings to perform meta-mappings with no additional parameters, by transforming the vector task representations.
    \end{itemize}
...
\item More info: \url{https://arxiv.org/abs/1905.09950} and \url{https://github.com/lampinen/HoMM}
\end{itemize}
\end{frame}

\begin{frame}{Reproducibility \& why you should implement everything twice}
Bugs that slipped past my unit testing, but that I found by reimplementing my code in a shared library, and comparing results across the two versions: 
\begin{itemize}
