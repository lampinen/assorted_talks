...
\begin{frame}[standout]
Tasks \(\bm =\) functions from inputs to outputs. 
\end{frame}
...
\begin{frame}[standout]
We want to model how we can adapt the task function.
\end{frame}
...
\begin{frame}[standout]
Transforming the task function \(=\) adapting. \\[1em]
Meta-mappings are functions that transform task functions.
\end{frame}
...
\end{column}
\end{columns}
\only<2>{\vspace{1em} Parsimonious \& homoiconic!}
...
\frametitle<1>{Categorization model changes}
\frametitle<2>{RL model changes}
\begin{columns}
\begin{column}{0.5\textwidth}
\only<1>{
\begin{itemize}
...
\item Deeper but more bottlenecked task network (3 hidden layers, 64 units each), which performs better on these tasks. 
\end{itemize}
}
\only<2> {
\begin{itemize}
...
\item Shared CNN for input, and shared Q-value decoder, task-specific network between.
...
\begin{frame}{A comment on model-based adaptation}
...
\item That is, they're offloading a substantial part of the problem to another system.
...
\begin{frame}[standout]
Starting from a good ``guess'' at the appropriate behavior results in faster learning, and reduces cumulative mistakes by an order of magnitude 
\end{frame}

\section{Rethinking the role of language}

\againframe<4>{homm_arch}

\begin{frame}{MM works with language-based task representations too!}
...
\item However, this work should still be explored in richer environments. Whether this perspective is computationally useful with data as rich as human experience is an open question.
\end{itemize}
